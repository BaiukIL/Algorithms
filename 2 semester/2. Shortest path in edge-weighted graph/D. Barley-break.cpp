...
std::vector<uint64_t> Position::SnakePath() const {
  ...
  for (uint16_t k = 0; k < size; ++k) {
    ...
    if (k % 2 == 0) {
      ...
      for (uint16_t j = i - 1 + size; i < start + size; ++i, --j) {
        path[i] = GetAt(j);
      }
    }
  }
...
bool Position::CanAttain(const Position &otherPos) const {
  return CountInversions(SnakePath()) % 2 == CountInversions(otherPos.SnakePath()) % 2;
}
